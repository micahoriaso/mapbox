...
const Map = (): ReactElement => {
  ...
  const [mapStyle, setMapStyle] = React.useState<string>(
    "mapbox://styles/micahoriaso/ck0y66q3n042q1cqszv749n02"
  );
  ...
  const handleChangeMapStyle = (
    ...
    setMapStyle(String(value && value));
  };

  const filterByType = (location: Location): boolean => {
  ...
  const filterByChallenges = (location: Location): boolean => {
    if (filters.challenges.length > 0) {
      return filters.challenges.includes(
  ...
  const filterByCountry = (location: Location): boolean => {
  ...
  const filterByRound = (location: Location): boolean => {
  ...
  const filterByIsShared = (location: Location): boolean => {
    if (filters.shared.length > 0) {
      return filters.shared.includes(
    ...
    filteredLocations = [
      ...allLocations.features.filter(
        location =>
          ...
          filterByChallenges(location) &&
          filterByCountry(location) &&
          filterByRound(location) &&
          filterByIsShared(location)
      )
    ];
  ...
  const handleOnchange = (
    ...
    const formattedValue = String(value && value);
    ...
    if (checked) {
      ...
      filters[formattedName] = filters[formattedName].filter(
        item => item !== value
      );
      setFilters({
        ...filters
      ...
      <div className="map-container">
        ...
        >
          ...
          {locationData.features.length > 0 &&
            locationData.features.map((location: Location, index: number) => (
              <Marker
                key={index}
              ...
              >
                <Icon
                  ...
                  name={
                    getPinIcon(location.properties.Facility_Service_type!).icon
                  }
                  color={
                    getPinIcon(location.properties.Facility_Service_type!).color
                  }
